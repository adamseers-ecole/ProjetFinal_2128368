...
--1) Les instruments qui valent plus que 500$ les plus anciens
SELECT *
FROM Instruments.Instrument
WHERE Instrument.PrixMoy > 500
ORDER BY Instrument.DateInvention

--2) Les instruments à vent d'une même tonalité
SELECT COUNT(I.InstrumentID), V.Tonalite
FROM Instruments.Instrument I
INNER JOIN Instruments.InstrumentVent V
ON I.InstrumentID = V.InstrumentID
GROUP BY V.Tonalite

--3) Les instruments à vent d'une même tonalité qui valent plus cher que la moyenne
SELECT I.InstrumentID, I.Nom, I.Modele, V.Tonalite
FROM Instruments.Instrument I
INNER JOIN Instruments.InstrumentVent V
ON I.InstrumentID = V.InstrumentID
--GROUP BY V.Tonalite
--HAVING AVG(I.PrixMoy) < I.PrixMoy

--4) Les instruments à vent qui contiennent de l'étain
SELECT I.InstrumentID, I.Nom, I.Modele, V.Tonalite, M.NomMateriau
FROM Instruments.Instrument I
INNER JOIN Instruments.InstrumentVent V
ON I.InstrumentID = V.InstrumentID
INNER JOIN Materiaux.Materiau M
ON M.MateriauID = I.MateriauID
WHERE M.NomMateriau = 'brass'

--5) Tous les instrument qui ne contiennent pas d'étain
SELECT I.InstrumentID, I.Nom, I.Modele
FROM Instruments.Instrument I
WHERE I.MateriauID != ALL (SELECT MateriauID
                           FROM Materiaux.Materiau
                           WHERE NomMateriau = 'brass')
...
--Création d'une vue
CREATE VIEW Instruments.vw_Instrument AS
SELECT I.InstrumentID AS ID, I.Nom, F.Nom, I.Modele, M.NomMateriau, I.PrixMoy AS [Prix moyen], I.DateInvention AS [Date d'invention]
FROM Instruments.Instrument I
...
--La fontion va retourner la tonalité d'un instrument à vent selon son ID
GO
CREATE FUNCTION Instruments.ufn_DonnerTonalite (@InstrumentID int)
RETURNS int
AS
BEGIN
	DECLARE @retour nvarchar(4);

	SELECT @retour = V.Tonalite
	FROM Instruments.InstrumentVent V
